...
``getInfo-Asset`` 接口响应数据 asset 数组元素为 Object 类型，其中包含如下参数：
...
参数说明列表中成员 signers 的元素为 Object 类型，其中包含如下参数：
...
参数说明列表中成员 typeThresholds 的元素为 Object 类型，其中包含如下参数：
...
（资产转移并）触发合约操作为 Promise 类型，接口调用方式为：
...
（BU资产的发送和）触发合约操作为 Promise 类型，该操作接口调用方式：
...
``sign`` 接口响应数据 signatures 元素为 Object 对象, 其中包含如下参数：
...
响应数据中 transactions 的元素为 Object 类型，其中包含如下参数：
...
+----------------+-------------------------+-----------------------+
|  transaction   | TransactionInfoObject  | 交易内容列表          |
+----------------+-------------------------+-----------------------+
| signatures     | SignatureObject        | 签名列表              |
+----------------+-------------------------+-----------------------+
| tx_size        | int64                   | 交易大小              |
+----------------+-------------------------+-----------------------+ 
...
transactions 中的 transaction 为 transactionInfoObject 类型，其中包含如下参数：
...
transactions 中的 signatures 为 signatureObject 类型，其中包含如下参数：
